  ...
  Section PΠ.
    ...
    Section pre_functorial.
      ...
      Proof.
        ...
        destruct s as [s₁ s₂], d as [d₁ d₂], d' as [d'₁ d'₂], m1 as [m₁₁ m₁₂], m2 as [m₂₁ m₂₂].
        change (fst (?a, ?b)) with a in *.
        change (snd (?a, ?b)) with b in *.
        Time pre_anihilate.
        Require Import Utf8.
        Notation mkNT := Build_NaturalTransformation.
        Notation mkF := Build_Functor.
        Notation "! x" := (FunctorFromTerminal _ x) (at level 0, format "'!' x").
        Check CommaCategory_Object.
        Notation "F / G" := (CommaCategory_Object F G).
        Notation "x / G" := (CommaCategory_Object (FunctorFromTerminal _ x) G).
        Notation "s → d" := (CommaCategory_Morphism s d).
        Local Open Scope morphism_scope.
        Notation "s ~> d" := (CommaCategory_Morphism s d).
        Notation "x '.β'" := (CCO_β x) (at level 0, format "x '.β'").
        Notation "x '.h'" := (CCM_h x) (at level 0, format "x '.h'").
        Require Import Program.
        Print Implicit ComposeFunctors_FCompositionOf.
        Notation "'∘_fcomp_law'" := ComposeFunctors_FCompositionOf.
        Notation "1" := (IdentityFunctor _).
        Notation "1" := (Identity _).
        Notation "1" := eq_refl.
        Notation "'idʳ'" := (@RightIdentity _ _ _).
        Notation "p '⁻¹'" := (eq_sym p) (at level 10, format "p '⁻¹'").
        Notation "1" := (IdentityNaturalTransformation _).
        Notation "'λ_F' ( ob ) ( mor )" := (@Build_Functor _ _ ob mor _ _) (at level 100).
        Notation "'λ_T' ( com )" := (@Build_NaturalTransformation _ _ _ _ com _) (at level 100).
        Local Open Scope morphism_scope.
        Notation "_\_induced_functor m" := (CosliceCategoryMorphismInducedFunctor _ _ _ m) (at level 0).
        (*
1 subgoals, subgoal 1 (ID 361)
  ...
  m₁₂ : Morphism D s₂ d₂
  m₂₁ : NaturalTransformation d₁ d'₁
  m₂₂ : Morphism D d₂ d'₂
  ============================
   {|
   LCCM_F := _\_induced_functor (m₂₂ ∘ m₁₂);
   LCCM_T := λ_T (λ c : d'₂ / F, m₂₁ (c).β ∘ m₁₁ (c).β) |} =
   {|
   LCCM_F := _\_induced_functor (m₁₂) ∘ _\_induced_functor (m₂₂);
   LCCM_T := λ_T (λ c : d'₂ / F, m₂₁ (c).β ∘ (d₁ ₁ 1 ∘ m₁₁ (c).β ∘ 1)) |}
(dependent evars:)

*)

        Notation "'λ_F' ( ob ) ( mor ) ( x ) ( y )" := (@Build_Functor _ _ ob mor x y) (at level 100).
        Notation "'λ_T' ( com ) ( x )" := (@Build_NaturalTransformation _ _ _ _ com x) (at level 100).
        Locate "∘₀".
        Print Implicit NaturalTransformation.NTComposeF_subproof.
        Notation "'∘₁-pf'" := (@NaturalTransformation.NTComposeT_subproof _ _ _ _ _).
        Notation "'∘₀-pf'" := (@NaturalTransformation.NTComposeF_subproof _ _ _ _ _ _ _).
        Print Implicit RightPushforwardAlong_pre_curried_MorphismOf_pre_subproof.
        Notation "'Π-pf'" := (RightPushforwardAlong_pre_curried_MorphismOf_pre_subproof).
        Notation "1" := (@GeneralizedIdentityNaturalTransformation _ _ _ _ _ _).
(*
1 subgoals, subgoal 1 (ID 361)
  ...
  ============================
   ...
   LCCM_F := _\_induced_functor (m₂₂ ∘ m₁₂);
   LCCM_T := λ_T (λ c : d'₂ / F, m₂₁ (c).β ∘ m₁₁ (c).β)
             (Π-pf s₂ (d':=d'₂) (λ_T (λ c : C, m₂₁ c ∘ m₁₁ c)(∘₁-pf m₂₁ m₁₁))
                (m₂₂ ∘ m₁₂)) |} =
   {|
   LCCM_F := _\_induced_functor (m₁₂) ∘ _\_induced_functor (m₂₂);
   LCCM_T := λ_T (λ c : d'₂ / F, m₂₁ (c).β ∘ (d₁ ₁ 1 ∘ m₁₁ (c).β ∘ 1))
             (∘₁-pf
                (λ_T (λ c : d'₂ / F, m₂₁ (c).β)(Π-pf d₂ (d':=d'₂) m₂₁ m₂₂))
                (λ_T (λ c : d'₂ / F, d₁ ₁ 1 ∘ m₁₁ (c).β ∘ 1)
                 (∘₁-pf
                    (λ_T (λ c : d'₂ / F, d₁ ₁ 1 ∘ m₁₁ (c).β)
                     (∘₀-pf
                        (λ_T (λ c : d₂ / F, m₁₁ (c).β)
                         (Π-pf s₂ (d':=d₂) m₁₁ m₁₂)) 1)) 1))) |}
(dependent evars:)
